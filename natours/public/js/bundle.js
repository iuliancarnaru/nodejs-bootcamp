...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoiaXVsaWFuY2FybmFydSIsImEiOiJjazlsZWIxOTkwNTNwM2twYm1xaml2MWUwIn0.Fhyt0qNKjpqT4WWfJPZGoQ';
  ...
  locations.forEach(function (location) {
    ...
    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(location.coordinates).setHTML("<p>Day ".concat(location.day, " ").concat(location.description, "</p>")).addTo(map); // extend map bound to include the markers
...
var stripe = Stripe('pk_test_JPsIUikjnhqZrYJzmDW3VzUN00bTtu0HrO');
...
if (userDataFrom) {
  userDataFrom.addEventListener('submit', function (event) {
    event.preventDefault();
